...
int main(int argc, char *argv[])
{
    ...
    Info<< "\nStarting time loop\n" << endl;
    {
		...
		PD = turbulence->nut()*mag(S)*mag(S)/ (0.09*turbulence->omega()*turbulence->k());//turbulence->nut()*(gradU && dev(twoSymm(gradU))) / (0.09*turbulence->omega()*turbulence->k());
		...
		q1 = mag(R)/ ( mag(S)+mag(R) );		//(max(mag(S)*mag(S),SMALL_S2)); // Ratio of excess rotation rate to strain rate     
		rd = (turbulence->nu()+turbulence->nut())/(0.41*0.41*wallDistance*wallDistance*mag(gradU));
		q2 = 1-Foam::tanh(pow(8*rd,3));
		...
		//q4 = mag(turbulence->R())/(turbulence->k()); //Ratio of total to 1/2 * normal Reynolds stresses (TKE)
                TuM = sqrt(2.0*turbulence->k()) / sqrt(1.4*R_c*T);
                Rew = max((mag(R)*wallDistance*wallDistance /(turbulence->nu()+turbulence->nut())),0.0);//Foam::log10(mag(R)*wallDistance*wallDistance /(turbulence->nu()+turbulence->nut()));
                Info<< "Calculating Rew fields for ML111" << nl << endl;
                Res = max((mag(S)*wallDistance*wallDistance /(turbulence->nu()+turbulence->nut())),0.0);

...
*/
    ...
    */
    ...
    Res = (mags*y_*y_/(nu_+this->nut_));
    //sw = (magw/(magw+mags));
    volScalarField timeScale_(1/(0.09*omega_));
    volSymmTensorField S1(symm((fvc::grad(this->U_)))/(mags+0.09*omega_));
    volTensorField W1( -skew((fvc::grad(this->U_)))/(magw+0.09*omega_) );
    lamda1_ = (tr(S1&S1));
    lamda2_ = -(tr(W1&W1));
    I1 = 0.5*(tr(S1&S1)*tr(S1&S1) - tr (S1&S1 & S1&S1));
    I2 = 0.5*(tr(W1&W1)*tr(W1&W1) - tr (W1&W1 & W1&W1));
    nk = (k_/(k_+ 0.5*mag(this->U_)*mag(this->U_)));
    TuM = sqrt(2.0*k_) / sqrt(1.4*p/this->rho_);
    volSymmTensorField Sp = dev( symm( fvc::grad(this->U_) ) );
    Mg = mag(Sp)*1.414*sqrt(k_)/(omega_*sqrt(1.4*p/this->rho_));
    PD = max(this->nut_*mags*mags / (0.09*omega_*k_)-1.2,0.0);
    PD = Foam::tanh(PD);
    xx = this->nut_/nu_;
    q3 = min((sqrt(k_)*y_/(50.0*nu_)),2.0);
    q2 = Foam::tanh(pow(8*( (nu_+this->nut_)/(0.41*0.41*y_*y_*mag( fvc::grad(this->U_) )) ),3));
    Fs = 0.5-0.5*Foam::tanh(50*tr(symm( fvc::grad(this->U_) ))*(max( 0.25*sqrt(k_)/0.09/omega_, pow(V,1.0/3.0)))/(sqrt(1.4*p/this->rho_))+5);
    volScalarField theta1(tr(S1&S1));
    volScalarField theta2(tr(S1&S1));
    volScalarField theta3(tr(S1&S1)); 
    volScalarField theta4(tr(S1&S1)); 
    volScalarField theta5(tr(S1&S1)); 
    volScalarField theta6(tr(S1&S1)); 
    volScalarField theta7(tr(S1&S1));
    volScalarField theta8(tr(S1&S1)); 
    volScalarField theta9(tr(S1&S1)); 
    volScalarField theta10(tr(S1&S1)); 
             
    scalar lamda1_minVal = 1e8;
    scalar lamda1_maxVal = -1e8;
    scalar lamda2_minVal = 1e8;
    scalar lamda2_maxVal = -1e8;
    scalar nk_minVal = 1e8;
    scalar nk_maxVal = -1e8;
    scalar TuM_minVal = 1e8;
    scalar TuM_maxVal = -1e8;
    scalar Mg_minVal = 1e8;
    scalar Mg_maxVal = -1e8;
    scalar xx_minVal = 1e8;
    scalar xx_maxVal = -1e8;
    scalar Fs_minVal = 1e8;
    scalar Fs_maxVal = -1e8;
    scalar sw_minVal = 1e8;
    scalar sw_maxVal = -1e8;
    scalar Rew_minVal = 1e8;
    scalar Rew_maxVal = -1e8;
    scalar Res_minVal = 1e8;
    scalar Res_maxVal = -1e8;
    ...
    /*
        for(int i=0;i<nCells;i++)
        {
           theta1[i] = (Fs[i]-Fs_minVal)/(Fs_maxVal-Fs_minVal);
           theta2[i] = (TuM[i]-TuM_minVal)/(TuM_maxVal-TuM_minVal);
           theta3[i] = (Mg[i]-Mg_minVal)/(Mg_maxVal-Mg_minVal);
           theta4[i] = (nk[i]-nk_minVal)/(nk_maxVal-nk_minVal);
           theta5[i] = (xx[i]-xx_minVal)/(xx_maxVal-xx_minVal);
           theta6[i] = (Rew[i]-Rew_minVal)/(Rew_maxVal-Rew_minVal);  
           theta7[i] = (Res[i]-Res_minVal)/(Res_maxVal-Res_minVal);                          
        }
        */

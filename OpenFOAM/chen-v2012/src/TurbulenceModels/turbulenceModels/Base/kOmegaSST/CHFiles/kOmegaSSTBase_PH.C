...
*/
    ...
    if(betannmodel > ifmodel)// and currentTime > ifmodeltime.value())
    {
    //////////////nn model
    ...
    */
    ...
    volScalarField Res = mags*y_*y_/nu_;
    //volScalarField svt(   (y_*mag(this->U_))/this->nut_ );
    volScalarField timeScale_(0.09*omega_);
    volSymmTensorField S(symm((fvc::grad(this->U_))));
    volTensorField W(-skew((fvc::grad(this->U_))));
    
    volSymmTensorField Shat(S/(mag(S)+timeScale_));
    volTensorField     What(W/timeScale_);
    
    volScalarField Q = mag(W)/ (mag(S)+mag(W));
    volScalarField lamda1_ = (tr(Shat&Shat));
    volScalarField lamda2_ = (tr(What&What));
    volScalarField nk = (k_/(k_+ 0.5*mag(this->U_)*mag(this->U_)));
    volScalarField theta1(tr(S&S));
    volScalarField theta2(tr(S&S));
    volScalarField theta3(tr(S&S)); 
    volScalarField theta4(tr(S&S)); 
    volScalarField theta5(tr(S&S)); 
    volScalarField theta6(tr(S&S)); 
    ...
    forAll(lamda1_, cellI)
    {
             ...
             lamda2_maxVal = max(lamda2_[cellI], lamda2_maxVal);
             Res_minVal = min(Res[cellI], Res_minVal);
             Res_maxVal = max(Res[cellI], Res_maxVal);
             Rew_minVal = min(Rew[cellI], Rew_minVal);
             Rew_maxVal = max(Rew[cellI], Rew_maxVal);
             nk_minVal = min(nk[cellI],nk_minVal);
             nk_maxVal = max(nk[cellI],nk_maxVal);
    ...
    //NORMALIZE
        ...
        scalar theta2_minVal = lamda2_minVal;
        scalar theta2_maxVal = lamda2_maxVal;
        scalar theta3_minVal = Rew_minVal;
        scalar theta3_maxVal = Rew_maxVal; 
        scalar theta4_minVal = nk_minVal;
        scalar theta4_maxVal = nk_maxVal; 
        scalar theta5_minVal = Res_minVal;
        scalar theta5_maxVal = Res_maxVal;
        scalar theta6_minVal = Q_minVal;
        scalar theta6_maxVal = Q_maxVal;
    ...
    //NORMALIZE      
        ...
        theta5[cellI] = (theta5[cellI]-theta5_minVal)/(theta5_maxVal-theta5_minVal);
        theta6[cellI] = (theta6[cellI]-theta6_minVal)/(theta6_maxVal-theta6_minVal);
        }

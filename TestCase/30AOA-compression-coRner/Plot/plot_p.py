...
SSTNN = np.loadtxt('p_walls_constant1.raw',skiprows=2)
a=100
b=140
...
plt.plot(SST[:, 0]/0.0072, SST[:, 3]/2509, 'r', lw=2, label='SST') #, markersize=2)
...
plt.plot(x_combined1,y_combined1, color='blue',linestyle='-.', linewidth=2,label=r'$Improved \,SST$') #, markersize=2) markersize=2)
...
ax_inset.plot(SST[:, 0]/0.0072, SST[:, 3]/2509, 'r', lw=2, label=f'$SST$') #, markersize=2)
ax_inset.plot(SSTNN[:, 0]/0.0072, SSTNN[:, 3]/2509, color='blue',linestyle='-.', linewidth=2,label=r'$Improved \,SST$') #, markersize=2)


# Set limits and labels for the inset
# Define your specific range for x and y axes for the inset
x_range = (-7, -2)  # Example x-axis range
y_range = (0, 10)  # Example y-axis range
...
plt.tight_layout()
plt.savefig('c_ramp_p.tiff',dpi=600,bbox_inches='tight')
plt.show()
